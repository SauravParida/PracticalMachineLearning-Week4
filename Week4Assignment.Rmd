---
title: "Practical Machine Learnig Week4 Assignment"
author: "sparida"
date: "21 March 2018"
...
```{r}
training<-read.csv("C:/Users/SOURAV/Documents/Coursera/PracticalMachineLearning/Week4/pml-training.csv", na.strings = c("NA", "#DIV/0!", ""))
testing<-read.csv("C:/Users/SOURAV/Documents/Coursera/PracticalMachineLearning/Week4/pml-testing.csv", na.strings = c("NA", "#DIV/0!", ""))
...
## Data Clean Up by Removing Columns
...
I'm now ready to create our model. I'm using a Random Forest algorithm, using the train() function from the caret package. I'm using 9 variables out of the 54 as model parameters. These variables were among the most significant variables generated by an initial Random Forest algorithm, and are roll_belt, num_window, pitch_belt, magnet_dumbbell_y, magnet_dumbbell_z, pitch_forearm, accel_dumbbell_y, roll_arm, and roll_forearm. These variable are relatively independent as the maximum correlation among them is 50.57%. I'm using a 2-fold cross-validation control. This is the simplest k-fold cross-validation possible and it will give a reduced computation time. Because the data set is large, using a small number of folds is justified.
...
The above line of code required 5.2 minutes to execute (selecting all 54 variables would increase this time to 7.1 minutes, an increase of 37%, without increasing the accuracy of the model), so we may want to save the model generated for later use:
